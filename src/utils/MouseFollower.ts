export const MouseFollower = (mouseFollower : HTMLDivElement) => {
    ...
    let circle = {
        ...
        update:function(){
            ...
            if (window.pageYOffset + this.el.getBoundingClientRect().top > window.innerHeight) {
            this.el.style.borderColor = "#E5CDC3"
            } else {
                this.el.style.borderColor = "#4E4C4B"
            }
        }
    }
    ...
    setInterval(() => {
        ...
        window.requestAnimationFrame(() => circle.update());
    }, 1000/60);
